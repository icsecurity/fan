...
so_init (int numflows)
{
  ...
  if (iterations == 1)
    {
      strncpy (filename, setFileName (CONTAF, spec_logdir), FILENAME_MAX);
      strncpy (resultname, setFileNameResults (CONTAF), FILENAME_MAX);
      writeLogFile (filename, STATUS_OK,
		    "Start computing Contaflussi.so module", -1);
    }
  if (spec_logdir != NULL)
    free (spec_logdir);
  return STATUS_OK;
...
so_process (nf_record_t * r_current, int current_flows, char *bucket_id)
{
  ...
  if (iterations == 1)
    {
      fprintf (fs, "Timeslot Bucket,Total flows \n");
    }
  fprintf (fs, "%s,%d \n", bucket_id, current_flows);
  fclose (fs);
  return STATUS_OK;
}
