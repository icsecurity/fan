...
so_init (int numflows)
{
  ...
  if (iterations == 1)
    {
      strncpy (filename, setFileName (RENYILOGNAMEDEP, spec_logdir),
	       FILENAME_MAX);
      strncpy (resultname, setFileNameResults (RENYILOGNAMEDEP),
	       FILENAME_MAX);
      writeLogFile (filename, STATUS_OK,
		    "Start computing Renyi_modded.so module", -1);
    }
  if (spec_logdir != NULL)
    free (spec_logdir);
  if (iterations == 1)
...
process (nf_record_t * r_current, int current_flows)
{
  ...
  void *module;
  // Open the .so
  get_result so_getResulttwodistre;
  ...
  dlerror ();
  // Function so_init
  so_getResulttwodistre = dlsym (module, "so_getResulttwodistre");
  if ((error = dlerror ()))
    {
      fprintf (stderr, "Couldn't find so_init: %s\n", error);
      return ERR_OPEN_SO;
    }
  pro = (*so_getResulttwodistre) ();
  return STATUS_OK;
}
