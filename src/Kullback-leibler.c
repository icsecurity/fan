...
so_init (int numflows)
{
  ...
  if (iterations == 1)
    {
      strncpy (filename, setFileName (KULLBACKLOGNAME, spec_logdir),
	       FILENAME_MAX);
      strncpy (resultname, setFileNameResults (KULLBACKLOGNAME),
	       FILENAME_MAX);
      writeLogFile (filename, STATUS_OK,
		    "Start computing Kullback-leibler.so module", -1);
    }
  if (spec_logdir != NULL)
    free (spec_logdir);
  if (iterations == 1)
